...
2a. 若一句话同时包含明确的创建/训练意图和车牌/编号等 OCR 关键词，必须优先进入创建/训练分支，不能仅因关键词重叠而误判成“提取最近识别结果”。
2b. 基于最近一次推理结果的 OCR 提取只应作为 follow-up 动作；若当前轮带有新附件，系统必须先按当前附件理解该请求。
2c. 当用户已启用保存的 LLM 配置时，系统可以先生成一份“以目标为中心”的执行计划：
   - 判断用户究竟是要问答、做单步控制台动作，还是继续一个更大的闭环流程
   - 选择让用户操作最少的执行路径
   - 在不绕过既有确认/安全门禁的前提下，串联多个相关后端动作来减少手工步骤
...
5a. 当操作员需要更强的可视化运行视角时，可进入 `/training/jobs/:jobId/cockpit`
   - 顶部摘要：任务名、状态、epoch、运行时长、最佳指标、设备
   - 阶段流：展示已完成、当前、待执行阶段
   - 中央主区域：指标曲线与资源监控
   - 右侧面板：自动调参候选、试跑状态、最优方案、参数应用切换
   - 底部：带时间戳的事件流 / 日志流
5b. 驾驶舱支持双模式：
