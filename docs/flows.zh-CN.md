...
3. 通过轻量页头切换语言；桌面端统一从左侧底部账户区或折叠 rail 头像菜单进入设置/退出登录，紧凑或移动端页头仍可作为登录入口兜底
...
11. 用户可通过桌面右键或移动端长按菜单执行打开/重命名/置顶/删除快捷操作
12. 菜单打开后可使用快捷键 `O/R/P/D` 快速执行对应动作
13. 用户可在 Pinned 分组内拖拽调整会话优先级
14. 用户在同一上下文中继续多轮对话
