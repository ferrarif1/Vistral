...
`smoke:worker-model-pull-encrypted` 可用环境变量：
- `RUNTIME_API_KEY`（默认 `smoke-yolo-public-key`）
- `DELIVERY_KEY`（默认 `smoke-delivery-key-123`）
- `TRAINING_DATASET_ID` / `TRAINING_DATASET_VERSION_ID` / `TRAINING_MODEL_ID`（默认 `d-2` / `dv-2` / `m-1`）
...
- `VISTRAL_DISABLE_SIMULATED_TRAIN_FALLBACK`（应用运行时默认开启；仅在你明确需要兼容回退时才显式设为 `0`）
- `VISTRAL_DISABLE_INFERENCE_FALLBACK`（应用运行时默认开启；仅在你明确需要兼容回退输出时才显式设为 `0`）
...
- `PUBLIC_RUNTIME_INFERENCE_MAX_BYTES`（默认 `10485760`，`/api/runtime/public/inference` 解码后输入字节上限）
- `PUBLIC_MODEL_PACKAGE_MAX_BYTES`（默认 `67108864`，`/api/runtime/public/model-package` 模型工件字节上限）
- `MODEL_DELIVERY_ENCRYPTION_KEY`（可选，`/api/runtime/public/model-package` 未显式传 `encryption_key` 时的兜底加密密钥）
- worker 侧加密模型下发相关变量：
  - `WORKER_RUNTIME_PUBLIC_BASE_URL`（可选；默认 `${CONTROL_PLANE_BASE_URL}/api/runtime/public`）
  - `WORKER_RUNTIME_PUBLIC_API_KEY`（可选；worker `/api/worker/models/pull-encrypted` 默认使用的 Bearer key）
  - `WORKER_MODEL_DELIVERY_ENCRYPTION_KEY`（可选；worker `/api/worker/models/pull-encrypted` 默认解密密钥）
  - `WORKER_MODEL_STORE_ROOT`（可选；worker 拉取模型本地落盘根目录）
  - `WORKER_MODEL_PACKAGE_DOWNLOAD_TIMEOUT_SECONDS`（默认 `30`；worker 调用控制面 `/api/runtime/public/model-package` 超时）
- 本地命令模板脚本目录：`scripts/local-runners/`
- 占位符示例：`{{python_bin}}`、`{{repo_root}}`、`{{job_id}}`、`{{dataset_id}}`、`{{task_type}}`、`{{metrics_path}}`、`{{output_path}}`

公共 Runtime 接口（Bearer runtime API key，无需会话/CSRF）：
- `POST /api/runtime/public/inference`
- `POST /api/runtime/public/model-package`
访问控制沿用 Runtime Settings 的 key 绑定策略（`model_version:*`、`model:*`、framework 级兜底），可配额与过期。

worker 模型部署接口（需 worker token 鉴权）：
- `POST /api/worker/models/pull-encrypted`
- worker 会调用 `/api/runtime/public/model-package`，在本地完成 AES-256-GCM 解密并落盘，同时写入部署元数据。
