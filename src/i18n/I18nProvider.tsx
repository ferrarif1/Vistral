...
const zhCNBase: Record<string, string> = {
  ...
  'Vistral Platform': 'Vistral 平台',
  'Professional Console': '专业控制台',
  'Pixel Lab': '像素实验室',
  'Pixel Lab active': '像素实验室已开启',
  'Console Mode': '控制台模式',
  'Switch to Pixel Lab': '切换到像素实验室',
  'Switch to console mode': '切换到控制台模式',
  Workspaces: '工作区',
  'Model Build': '模型构建',
  'Data & Run': '数据与运行',
  Governance: '治理',
  ...
  'The highest-priority visible task is ready for review or one explicit operator action.':
    '最高优先级的可见任务已经可以审阅，或等待一次明确的操作人动作。',
  Understand: '理解需求',
  'Requirement captured.': '需求已捕获。',
  'Prepare data': '准备数据',
  'Dataset is not linked yet.': '尚未关联数据集。',
  'Run training': '执行训练',
  'Waiting for the next agent decision.': '正在等待下一步 Agent 决策。',
  'Promote or improve': '发布或改进',
  'Promotion evidence will appear after evaluation.': '评测完成后会出现发布证据。',
  'Task type': '任务类型',
  Metric: '指标',
  Gate: '门禁',
  'Open task': '打开任务',
  'Loading Pixel Lab': '加载像素实验室',
  'Preparing the pixel house from datasets, models, training jobs, and inference runs.':
    '正在根据数据集、模型、训练任务与推理运行搭建像素房子。',
  'Pixel Lab unavailable': '像素实验室暂不可用',
  'Pixel Lab status': '像素实验室状态',
  'Vistral Model House': 'Vistral 模型小屋',
  'Pixel house': '像素房子',
  'Current process': '当前过程',
  'Dataset warehouse is sorting albums': '数据集仓库正在整理相册',
  'Prepare or version a dataset before the model characters can study.':
    '先准备或快照化数据集，模型角色才能开始学习。',
  'Data sorting': '整理数据',
  'Model characters are choosing study material': '模型角色正在选择学习材料',
  'A ready dataset or version exists. Pick the next recipe-backed training run.':
    '已有可用数据集或版本。下一步可以选择由 recipe 支撑的训练运行。',
  Design: '设计',
  'Training workshop is active': '训练工坊正在运转',
  'A model character is learning from the selected dataset. Watch the training lane for logs and evidence.':
    '模型角色正在学习选中的数据集。请在训练泳道查看日志和证据。',
  Learning: '学习中',
  'Exam room is ready for inference validation': '推理考试房已准备好',
  'Choose a dataset and model version, then send the character into an explicit validation exam.':
    '选择数据集和模型版本，然后用显式验证动作让角色参加考试。',
  Exam: '考试',
  'Delivery room has registered model versions': '交付房已有注册模型版本',
  'Promotion evidence exists. Continue into model versions or run an exam before shipping.':
    '已有发布证据。可以进入模型版本页继续，或在交付前再跑一次考试。',
  Delivery: '交付',
  'Dataset Warehouse': '数据集仓库',
  'Albums and tapes become trainable snapshots here.': '相册和录像带会在这里变成可训练快照。',
  'Open Datasets': '打开数据集',
  'Model Character Studio': '模型角色工作室',
  'Base model characters wait here for a recipe and a dataset.':
    '基座模型角色在这里等待 recipe 和数据集。',
  'Latest job {{job}} is {{status}}.': '最新任务 {{job}} 当前为 {{status}}。',
  'No training run is active yet.': '当前还没有活跃训练运行。',
  'Inference Exam Room': '推理考试房',
  'Auto select': '自动选择',
  'Start exam': '开始考试',
  'Version Delivery Room': '版本交付房',
  'Latest registered version: {{version}}.': '最新注册版本：{{version}}。',
  'Registered versions will be packed here after promotion.': '模型通过发布后，注册版本会在这里打包。',
  'The house is synced from real Vistral records. Canonical actions still happen in the standard workflow pages.':
    '这座房子会同步真实 Vistral 记录。标准工作流页面仍然负责执行正式动作。',
  'Sort data': '整理数据',
  'Choose character': '选择角色',
  'Learn dataset': '学习数据集',
  'Take exam': '参加考试',
  'Deliver version': '交付版本',
  'Open latest vision task': '打开最新视觉任务',
  ...
  'Bundle Import': '数据包导入',
  'Select a local folder or ZIP package, then let Vistral organize upload, label import, and training preparation.':
    '选择本地文件夹或 ZIP 数据包，让 Vistral 自动串起上传、标注导入与训练准备。',
  'Inspect bundle': '检查数据包',
  'Upload images': '上传图片',
  'Import labels': '导入标注',
  'Prepare training': '准备训练',
  'Bundle workflow': '数据包流程',
  'Inspect first, then run one guided import.': '先检查内容，再执行一次引导式导入。',
  'Choose Folder': '选择文件夹',
  'Choose ZIP': '选择 ZIP',
  'Folder import reads image files plus supported labels (YOLO / COCO / LabelMe / OCR). ZIP import is unpacked locally in the browser before upload.':
    '文件夹导入会读取图片和支持的标注文件（YOLO / COCO / LabelMe / OCR）；ZIP 会先在浏览器本地解包后再上传。',
  'Bundle import failed': '数据包导入失败',
  'Annotation files': '标注文件',
  'Duplicate names': '重名数量',
  'Bundle label format': '数据包标注格式',
  'No supported labels found': '未发现支持的标注文件',
  'Images can still be uploaded, but no importable labels were detected in this bundle.':
    '仍可上传图片，但当前数据包里没有检测到可导入的标注内容。',
  'After import, auto-run split and create a dataset version snapshot':
    '导入后自动执行切分并创建数据集版本快照',
  'Training snapshot prepared': '训练快照已准备',
  'Dataset version {version} is ready for the next training step.':
    '数据集版本 {version} 已可进入下一步训练。',
  'Open Training Launch': '打开训练启动页',
  'Run Bundle Import': '执行数据包导入',
  'No bundle selected': '尚未选择数据包',
  'Choose a local folder or ZIP package to inspect images and annotation files before import.':
    '先选择本地文件夹或 ZIP 数据包，再检查其中的图片和标注文件。',
  'Local folder': '本地文件夹',
  'No image files were found in the selected bundle.': '所选数据包中没有识别到图片文件。',
  'Duplicate image filenames were found. Keep image basenames unique before import.':
    '检测到图片文件名重名。请保证图片 basename 唯一后再导入。',
  'Uploaded attachment disappeared before import could continue.': '导入继续前，刚上传的附件已不可见。',
  'Attachment import source failed during processing.': '导入来源附件在处理中失败。',
  'Attachment did not become ready in time. Refresh and retry.': '附件在预期时间内未进入 ready，请刷新后重试。',
  'Bundle import finished. Uploaded {images} image files, imported {records} labeled records, and prepared dataset version {version}.':
    '数据包导入完成：上传图片 {images} 个，导入标注记录 {records} 条，并已准备数据集版本 {version}。',
  'Bundle import finished and dataset version {version} was created, but training readiness is still blocked by split or annotation coverage.':
    '数据包导入完成，并已创建数据集版本 {version}，但训练就绪仍被切分或标注覆盖率阻塞。',
  'Bundle import finished. Uploaded {images} image files and imported {records} labeled records.':
    '数据包导入完成：上传图片 {images} 个，导入标注记录 {records} 条。',
  'No YOLO annotation file was found in the selected bundle.': '所选数据包中没有找到 YOLO 标注文件。',
  'No YOLO annotations could be paired with uploaded images.': '没有任何 YOLO 标注能与已上传图片成功配对。',
  'No OCR annotations could be paired with uploaded images.': '没有任何 OCR 标注能与已上传图片成功配对。',
  'No LabelMe JSON annotations were found in the selected bundle.': '所选数据包中没有找到 LabelMe JSON 标注。',
  'No COCO annotation JSON was found in the selected bundle.': '所选数据包中没有找到 COCO 标注 JSON。',
...
const zhCNPageExtras: Record<string, string> = {
  ...
  'Vistral is assembling this run': 'Vistral 正在组装这次训练',
  'Goal': '目标',
  'Describe the model target, or launch from selected defaults.': '描述模型目标，或沿用当前默认方案启动。',
  'Snapshot': '快照',
  'Smart Launch can prepare a snapshot from this dataset.': '智能启动可以基于该数据集准备快照。',
  'Choose a dataset version snapshot.': '请选择一个数据集版本快照。',
  'Plan': '方案',
  'Recipe-backed plan is loading or unavailable.': '基于 recipe 的方案正在加载或暂不可用。',
  'Readiness': '就绪度',
  'Checking dataset, runtime, workers, and evidence.': '正在检查数据集、运行时、worker 与证据。',
  'Waiting for a recipe-backed snapshot.': '等待基于 recipe 的数据快照。',
  'Complete': '完成',
  'Blocked': '阻塞',
  'Warning': '警告',
  'Diagnostics': '诊断',
