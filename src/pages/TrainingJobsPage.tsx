    ...
    <div className="stack">
      <div className="row between gap align-center">
        <div className="stack tight">
          <h2>Training Jobs</h2>
          <small className="muted">Track job states, logs, and metrics for OCR and detection tasks.</small>
        </div>
      ...
      <section className="console-grid">
        <article className="card stack tight">
          ...
          <small className="muted">Running</small>
        </article>
        <article className="card stack tight">
          <strong className="metric">{summary.completed}</strong>
          <small className="muted">Completed</small>
        </article>
        <article className="card stack tight">
          <strong className="metric">{summary.failed}</strong>
          <small className="muted">Failed/Cancelled</small>
        </article>
        <article className="card stack tight">
      ...
      {loading ? <StateBlock variant="loading" title="Loading Jobs" description="Fetching training jobs." /> : null}
      {error ? <StateBlock variant="error" title="Load Failed" description={error} /> : null}

      {!loading && !error && jobs.length === 0 ? (
        <StateBlock variant="empty" title="No Jobs" description="Create your first training job." />
      ) : null}
