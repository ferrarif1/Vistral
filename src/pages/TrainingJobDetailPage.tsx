    ...
    <Card className="workspace-overview-hero">
      <div className="workspace-overview-hero-grid">
        <div className="workspace-overview-copy stack">
          <small className="workspace-eyebrow">{t('Training Detail')}</small>
          <h1>{t('Training Job Detail')}</h1>
          <p className="muted">
            {job ? `${job.name} · ${t(job.task_type)} · ${t(job.framework)}` : t('Review status, logs, and metrics for the selected training run.')}
          </p>
        </div>
        <div className="workspace-overview-badges">
          <div className="workspace-overview-badge">
            <span>{t('Status')}</span>
            <strong>{job ? t(job.status) : t('Pending')}</strong>
          </div>
          <div className="workspace-overview-badge">
            <span>{t('Metrics')}</span>
      ...
      <section className="workspace-overview-signal-grid">
        <Card as="article" className="workspace-signal-card">
          <div className="workspace-signal-top">
            <h3>{t('Run status')}</h3>
            <small className="muted">{t('Current lifecycle stage for this training job.')}</small>
          </div>
          <strong className="metric">{t(job.status)}</strong>
        </Card>
        <Card as="article" className="workspace-signal-card">
          <div className="workspace-signal-top">
            <h3>{t('Metrics')}</h3>
            <small className="muted">{t('Recorded metric points from this run timeline.')}</small>
          </div>
          <strong className="metric">{metrics.length}</strong>
        </Card>
        <Card as="article" className="workspace-signal-card">
          <div className="workspace-signal-top">
            <h3>{t('Logs')}</h3>
            <small className="muted">{t('Captured execution log lines for the selected run.')}</small>
          </div>
          <strong className="metric">{logs.length}</strong>
        </Card>
        <Card as="article" className={`workspace-signal-card${artifactAttachmentId ? '' : ' attention'}`}>
          <div className="workspace-signal-top">
            <h3>{t('Artifact')}</h3>
            <small className="muted">{t('Artifact attachment generated by this run.')}</small>
          </div>
          <strong className="metric">{artifactAttachmentId ? t('Ready') : t('pending')}</strong>
        </Card>
      </section>
                  ...
                  <details className="workspace-details">
                    ...
                    <div className="stack tight">
                      {schedulerDecisionHistory.map((decision, index) => (
                        <Panel key={`${decision.decided_at}-${decision.trigger}-${decision.attempt}-${index}`} tone="soft" className="stack tight">
                          <div className="row between align-center wrap">
                            <strong>
