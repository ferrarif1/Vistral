...
3. 阅读产品合同文档（英文原版）：
  ...
  - `docs/agent-training-orchestration.md`
   对应中文镜像版：
  - `docs/prd.zh-CN.md`
  - `docs/ia.zh-CN.md`
  - `docs/flows.zh-CN.md`
...
4. 若你是在延续进行中的工程任务，继续读这两份执行文档：
  - `PLANS.md`
  - `docs/work-handoff.md`
5. 按 `docs/deployment.docker.md` 使用唯一的 Docker 部署入口。
6. 若新任务会打断当前未完成工作，先在 `docs/work-handoff.md` 追加交接记录再切换上下文。

## Repository Working Model（本仓库 Codex 工作方式）

- 协作与执行规则：`AGENTS.md`
- 产品与工程合同：`docs/*`
- 当前路线图与分轮执行记录：`PLANS.md`、`docs/work-handoff.md`
...
5. 执行 `npm run docker:verify:full`
  - 已内置 OCR fallback 安全守卫：本地 OCR 命令失败时必须返回空识别结果并带明确 fallback 标记，禁止出现硬编码业务文本
...
- `npm run data:reset:foundation`（将持久化 app-state 重置到最小基线：保留账号与基座模型，清理数据集/训练/推理测试与种子运行数据）
  - 默认会同时清理本地运行时存储目录（`UPLOAD_STORAGE_ROOT`、`TRAINING_WORKDIR_ROOT`、`MODEL_EXPORT_ROOT`、`.data/runtime-local-predict`）
  - 如只想重置 app-state 不清理文件，可设置 `RESET_FOUNDATION_PURGE_STORAGE=0`
- `npm run smoke:foundation-reset`（同时验证两条守护：`data:reset:foundation` 清理后仅保留基座模型；`APP_STATE_BOOTSTRAP_MODE=minimal` 首次启动不注入数据集/训练/推理种子记录）
- `npm run smoke:adapter-no-placeholder`（验证适配器防占位保证：`evaluate()` 读取真实指标文件或返回空指标、`export()` 返回真实本地产物路径、`load_model()` 在产物缺失时必须显式失败）
- `npm run smoke:training-template-guard`（验证 bundled 本地训练 runner 在 template 模式必须输出显式非真实证据：`mode=template`、`training_performed=false`、且 `fallback_reason`/`template_reason` 非空）
- `npm run typecheck`
- `npm run lint`
- `npm run build`
...
- `npm run smoke:conversation-next-steps-ui-wiring`（守住完整对话页与右侧 Dock 都渲染共享建议下一步，并保留受控 `/ops` 输入）
- `npm run smoke:conversation-next-steps-browser`（使用本地 Chrome/CDP 对完整对话页与右侧 Dock 做真实点击巡检；脚本使用临时 app-state，并把截图保存到 `.data/verify-reports/`）
- `npm run smoke:conversation-ops-bridge`（验证自然语言 ops 桥接闭环：意图路由、缺参追问、后续补参续跑与高危确认门禁）
  - 默认模式下，高危分支只校验确认门禁，不执行最终 mutation，避免本地数据/runtime 状态被 smoke 污染；如需强校验最终执行可加 `SMOKE_OPS_BRIDGE_EXECUTE_MUTATIONS=true`
- `npm run smoke:runtime-profile-activation`（验证运行时 profile 激活闭环：环境注入 profile 可见、`active_profile_id` 切换，以及激活后的框架配置投影到返回视图）
...
- `npm run smoke:core-closure`（一键串行执行核心闭环回归：`no-seed-hardcoding` + `foundation-reset` + `adapter-no-placeholder` + `training-template-guard` + `model-version-register-gate` + `account-governance` + `phase2` + `runtime-success` + `conversation-actions` + `inference-feedback-guard` + `real-closure` + `ocr-closure` + `training-worker-dedicated-auth`）
- `npm run smoke:llm-settings`（验证 LLM 设置的保存、保留旧 key 编辑、重启后加密读回与清空流程）
- `npm run smoke:runtime-settings-persistence`（验证 Runtime 设置的保存、重载、清空回退流程：包含重启后加密持久化，以及清空后回落到环境变量默认值）
- `npm run smoke:runtime-success`（通过本地 runtime mock 验证 YOLO/PaddleOCR/docTR 成功路径）
- `npm run smoke:ocr-fallback-guard`（验证 OCR 在本地命令失败或 runtime 回退时不再返回误导性业务文本；应返回空 OCR 结果并携带 fallback reason/source）
...
- `npm run smoke:ocr-closure`（验证独立 OCR 闭环：OCR 导入 -> PaddleOCR/docTR 本地命令训练 -> 指标/产物摘要 -> 模型版本注册 -> 推理上传与执行）
  ...
  - 若需要强制“非 template / 非 fallback”真实证据断言，可执行 `OCR_CLOSURE_REQUIRE_REAL_MODE=true npm run smoke:ocr-closure`（会自动启用 `VISTRAL_RUNNER_ENABLE_REAL=1`）
  - 默认优先使用 `.data/runtime-python/.venv/bin/python`（若存在）；可通过 `PYTHON_BIN=/path/to/python` 覆盖
  - 为提升真实模式稳定性，脚本默认会生成可控 OCR 文本样本图（可通过 `OCR_CLOSURE_GENERATE_TEXT_SAMPLE=false` 关闭）
  - 首次拉取/加载模型较慢时，可通过 `OCR_CLOSURE_WAIT_POLLS` 与 `OCR_CLOSURE_WAIT_SLEEP_SEC` 调整等待窗口
- `npm run smoke:real-closure`（验证完整 real 闭环：需求草拟 -> 数据集上传/导入/导出 -> YOLO 训练 -> 模型版本注册 -> YOLO/PaddleOCR/docTR 推理 -> detection/OCR 反馈回流与数据集追踪）
  - 默认优先使用 `.data/runtime-python/.venv/bin/python`（若存在）；可通过 `PYTHON_BIN=/path/to/python` 覆盖
  - 为提升 OCR 训练/推理稳定性，脚本默认会生成可控 OCR 文本样本图（可通过 `REAL_CLOSURE_GENERATE_TEXT_SAMPLE=false` 关闭）
  - 若需要强制“非 template / 非 fallback”的真实 OCR 证据断言，可执行 `REAL_CLOSURE_REQUIRE_REAL_MODE=true npm run smoke:real-closure`（会自动启用 `VISTRAL_RUNNER_ENABLE_REAL=1`）
  - 首次模型预热较慢时，可通过 `REAL_CLOSURE_YOLO_WAIT_POLLS`/`REAL_CLOSURE_YOLO_WAIT_SLEEP_SEC` 与 `REAL_CLOSURE_DOCTR_WAIT_POLLS`/`REAL_CLOSURE_DOCTR_WAIT_SLEEP_SEC` 调整等待窗口
...
- `npm run doctor:real-training-readiness`（检查当前机器是否满足真实本地训练/推理分支依赖：`ultralytics`/`paddleocr`/`doctr` 与本地 YOLO 权重路径，推荐 `YOLO_LOCAL_MODEL_PATH`）
- `npm run setup:real-training-env`（一键准备真实分支依赖：创建本地 Python venv、安装依赖，并尝试拉取 YOLO 权重用于 real 模式验证）
...
- 可通过 `APP_STATE_BOOTSTRAP_MODE` 配置首次启动种子模式：
  - `full`（默认）：保留原型种子基线
  - `minimal`：当不存在 app-state 文件时，仅初始化账号与基座模型
...
- 可选本地命令适配：
  ...
  - 可选内置 runner Python 覆盖：`VISTRAL_PYTHON_BIN`（回退顺序：`PYTHON_BIN`，再按平台默认 `python3`/`python`）
  - 超时控制：`LOCAL_RUNNER_TIMEOUT_MS`
  - 严格禁用 simulated 训练回退：`VISTRAL_DISABLE_SIMULATED_TRAIN_FALLBACK=1` 在应用运行时默认开启；只有在你明确需要兼容回退时才显式设为 `0`
  - 严格禁用推理 fallback：`VISTRAL_DISABLE_INFERENCE_FALLBACK=1` 在应用运行时默认开启；只有在你明确需要兼容回退输出时才显式设为 `0`
  - 可选真实 runner 开关：`VISTRAL_RUNNER_ENABLE_REAL=auto`（默认自动尝试真实执行；设为 `0` 可强制 template 模式）
  - 可选真实 runner 参数：
    - `PADDLEOCR_LOCAL_MODEL_PATH`
    - `DOCTR_LOCAL_MODEL_PATH`
    - `YOLO_LOCAL_MODEL_PATH`（推荐；仍兼容旧的 `VISTRAL_YOLO_MODEL_PATH` / `REAL_YOLO_MODEL_PATH`）
  ...
  - 模板占位符包含：`{{python_bin}}`、`{{repo_root}}`、`{{job_id}}`、`{{dataset_id}}`、`{{task_type}}`、`{{metrics_path}}`、`{{output_path}}`
  - worker 分发相关：
...
9. `npm run smoke:plan-llm-complete` 会在 `.data/verify-reports/` 生成 `plan-llm-complete-<timestamp>.json/.md/.log`，默认保留最近 10 组（可用 `PLAN_LLM_REPORT_RETAIN` 调整）；同一命令已接入 `.github/workflows/plan-llm-complete.yml`，支持手动与夜间 CI 运行；可用 `npm run smoke:plan-llm-ci-workflow` 在本地守住 workflow 与报告产物路径。
