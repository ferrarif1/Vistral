...
- 可选本地命令适配：
  ...
  - 严格禁用 simulated 训练回退：`VISTRAL_DISABLE_SIMULATED_TRAIN_FALLBACK=1`（当本地 runner 命令缺失/不可用时直接失败）
  - 严格禁用推理 fallback：`VISTRAL_DISABLE_INFERENCE_FALLBACK=1`（runtime/local predict 必须返回真实结果；template/fallback 会被直接拒绝）
