...
- `npm run smoke:core-closure`（一键串行执行核心闭环回归：`no-seed-hardcoding` + `account-governance` + `phase2` + `conversation-actions` + `inference-feedback-guard` + `real-closure` + `ocr-closure`）
...
7. 全链路验收：`npm run docker:verify:full`
   - 覆盖 auth/权限、账号治理校验、真实 multipart 附件链路、对话侧真实创建动作（数据集/模型草稿/训练任务）、审批+推理反馈、Phase2 标注复审与训练发起门禁、数据集导入导出 roundtrip（detection/ocr/segmentation）、detection real-closure 与 OCR closure
