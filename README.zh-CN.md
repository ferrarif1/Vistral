...
- `npm run smoke:core-closure`（一键串行执行核心闭环回归：`no-seed-hardcoding` + `foundation-reset` + `adapter-no-placeholder` + `training-template-guard` + `model-version-register-gate` + `account-governance` + `phase2` + `conversation-actions` + `inference-feedback-guard` + `real-closure` + `ocr-closure` + `training-worker-dedicated-auth`）
...
- `npm run doctor:real-training-readiness`（检查当前机器是否满足真实本地训练/推理分支依赖：`ultralytics`/`paddleocr`/`doctr` 与 `VISTRAL_YOLO_MODEL_PATH`）
