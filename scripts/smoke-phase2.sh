...
YOLO_RUNTIME_ENDPOINT="$YOLO_RUNTIME_ENDPOINT_FOR_SMOKE" npm run dev:api >"$LOG_FILE" 2>&1 &
API_PID=$!

for _ in $(seq 1 30); do
  if curl -sS "http://127.0.0.1:${API_PORT}/api/health" >/dev/null 2>&1; then
    break
  fi
  sleep 0.2
done
...
csrf_payload="$(curl -sS -c "$COOKIE_FILE" -b "$COOKIE_FILE" "http://127.0.0.1:${API_PORT}/api/auth/csrf")"
...
created_dataset="$(curl -sS -c "$COOKIE_FILE" -b "$COOKIE_FILE" \
  ...
  "http://127.0.0.1:${API_PORT}/api/datasets")"
...
upload_attachment="$(curl -sS -c "$COOKIE_FILE" -b "$COOKIE_FILE" \
  ...
  "http://127.0.0.1:${API_PORT}/api/files/dataset/${dataset_id}/upload")"
...
dataset_detail="$(curl -sS -c "$COOKIE_FILE" -b "$COOKIE_FILE" "http://127.0.0.1:${API_PORT}/api/datasets/${dataset_id}")"
...
upsert_result="$(curl -sS -c "$COOKIE_FILE" -b "$COOKIE_FILE" \
  ...
  "http://127.0.0.1:${API_PORT}/api/datasets/${dataset_id}/annotations")"
...
annotations_list="$(curl -sS -c "$COOKIE_FILE" -b "$COOKIE_FILE" "http://127.0.0.1:${API_PORT}/api/datasets/${dataset_id}/annotations")"
...
inference_result="$(curl -sS -c "$COOKIE_FILE" -b "$COOKIE_FILE" \
  ...
  -H "x-csrf-token: $csrf_token" \
  -d "$inference_request" \
  "http://127.0.0.1:${API_PORT}/api/inference/runs")"
...
echo "fallback_source=${fallback_source}"
