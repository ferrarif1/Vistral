...
if ! command -v python3 >/dev/null 2>&1; then
  echo "[smoke-inference-feedback-guard] python3 is required."
  exit 1
fi
...
if ! wait_for_health; then
  if [[ "${START_API}" == "true" ]]; then
    if ! kill -0 "${API_PID}" >/dev/null 2>&1; then
      echo "[smoke-inference-feedback-guard] API process exited before health check (possible port conflict)."
      cat "${API_LOG}"
      exit 1
    fi
