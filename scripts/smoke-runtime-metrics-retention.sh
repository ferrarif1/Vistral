...
if ! kill -0 "${API_PID}" >/dev/null 2>&1; then
  echo "[smoke-runtime-metrics-retention] API process exited before health check (possible port conflict)."
  cat "${API_LOG}"
  exit 1
fi
...
if [[ -z "${detection_dataset_id}" ]]; then
  echo "[smoke-runtime-metrics-retention] no detection dataset found."
  echo "${datasets_resp}"
  exit 1
fi
...
if [[ -z "${detection_dataset_version_id}" ]]; then
  echo "[smoke-runtime-metrics-retention] no trainable detection dataset version found."
...
job_resp="$(curl -sS -c "${COOKIE_FILE}" -b "${COOKIE_FILE}" \
  ...
  -d "{\"name\":\"retention-cap-test\",\"task_type\":\"detection\",\"framework\":\"yolo\",\"dataset_id\":\"${detection_dataset_id}\",\"dataset_version_id\":\"${detection_dataset_version_id}\",\"base_model\":\"yolo11n\",\"config\":{\"epochs\":\"12\",\"batch_size\":\"2\",\"learning_rate\":\"0.0007\"}}")"
