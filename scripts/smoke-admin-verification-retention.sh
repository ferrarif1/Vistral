...
if ! command -v python3 >/dev/null 2>&1; then
  echo "[smoke-admin-verification-retention] python3 is required."
  exit 1
fi
...
if ! kill -0 "${API_PID}" >/dev/null 2>&1; then
  echo "[smoke-admin-verification-retention] API process exited before health check (possible port conflict)."
  cat "${API_LOG}"
  exit 1
fi
