...
if [[ "${START_API:-true}" == "true" && -z "${API_PORT:-}" ]]; then
  if ! command -v python3 >/dev/null 2>&1; then
    echo "[smoke-dataset-export-roundtrip] python3 is required when auto-selecting API_PORT."
    exit 1
  fi
  API_PORT="$(
    python3 - <<'PY'
...
if [[ "${START_API}" == "true" ]] && ! kill -0 "${API_PID}" >/dev/null 2>&1; then
  echo "[smoke-dataset-export-roundtrip] API process exited before health check (possible port conflict)."
  cat "${API_LOG}"
  exit 1
fi
